...
// let k;
 const Audios = () =>{
      ...
      const files = useSelector((stateDate) => stateDate.fileList);
        // console.log(files.fileList[0]?.createdAt.split('-')[2].split('T')[0])
        ...
        const [audioList, setAudioList] = useState(initialValue);
          const someList=()=>{
           ...
           for(let i=(files?.fileList?.length)-1;i>=0;i--){
                    ...
                    if(files?.fileList[i]?.type=="audio")
                    setAudioList(list => list.concat({id:files?.fileList[i]?.id,name:files?.fileList[i]?.name,fileName:files.fileList[i]?.name.split('-')[2],createdAt:files?.fileList[i]?.createdAt.split('T')[0]}));
                    }
        ...
        for(let i=(files?.fileList?.length)-1;i>=0;i--){
          if(!selectedValue){
            ...
            break;
             ...
             if(files.fileList[i]?.createdAt.split('-')[0]==selectedValue && files?.fileList[i]?.type=="audio")
             setScheduledSearches(scheduledSearches => scheduledSearches.concat({id:files?.fileList[i]?.id,name:files?.fileList[i]?.name,createdAt:files?.fileList[i]?.createdAt.split('T')[0],fileName:files.fileList[i]?.name.split('-')[2]}));
            }}
          }
     ...
     const handleDownloadAudio = async (name) => {
     saveAs(`http://localhost:11278/audios/${name}`, 'audio.mp3') // Put your image url here.
  ...
  <>
    ...
    <button className="bg-slate-50 w-20  h-10 rounded-md" 
     ...
     <div className="flex ">
      <select className="  h-12 w-24 bg-zinc-100 text-center m-0.5 font-bold"
      value={selectedValue}
     ...
     {/* Date partition List  */}
   <br /><br />
   <div className=" lg:ml-1 flex flex-wrap mt-5 sm:ml-1   ">   
              {
         (scheduledSearches.length)>0
          ?(
            scheduledSearches.map((file) => (
           <div className=" mt-5 mb-10 flex w-80 h-20 m-3 flex-wrap"key={file.id}>
           <audio  controls className=" peer hover  ml-5 h-16 p-1 mt-5  rounded-2xl justify-center"
             src={`${BaseUrl}/audios/${file.name}`}  type="audio/mp3">
           </audio> 
          <span className="ml-10"> {file.fileName} </span>
          ...
          <div class="hidden peer-hover:flex hover:flex absolute
           w-[70px]
           flex-col  h-14 ml-48  mt-20 bg-slate-100 rounded-lg p-1">
          <button  className=" h-4 w-4 text-center bold ml-2 cursor-pointer m-7 -mt-2"
